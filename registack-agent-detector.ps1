...
function Normalize-ScanDirArgs {
    param([string[]]$ArgsIn)
    ...
    while ($i -lt $ArgsIn.Count) {
        $current = $ArgsIn[$i]
        $output.Add($current)
        if ($current -eq "--scan-dir" -and ($i + 1) -lt $ArgsIn.Count) {
            $nextArg = $ArgsIn[$i + 1]
            try {
...
$pythonScript = Join-Path $scriptRoot "registack-agent-detector.py"
if (-not (Test-Path -LiteralPath $pythonScript)) {
    throw "Python detector script not found next to wrapper: $pythonScript"
}

$pythonCommand = Resolve-PythonCommand
$normalizedArgs = Normalize-ScanDirArgs -ArgsIn $RemainingArgs
